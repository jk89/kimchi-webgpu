...
async function main() {
  ...
  const failures = await page
    .waitForFunction(
      () => (window as any).testsFinished === true ? (window as any).testsFailures || 0 : false,
      { polling: 100, timeout: 0 }
    )
    .then(() => page.evaluate(() => (window as any).testsFailures || 0));

  console.log(`Tests finished. Failures: ${failures}`);
